...
from datasets import ParsableDataset
from trec import read_qrels_trec, read_top_trec
...
def get_queries(queries_f: Path) -> Dict[str, str]:
    queries = {}
    with open(queries_f, 'r')as f:
        for q_id, query in csv.reader(f, delimiter='\t'):
            queries[q_id] = query
    return queries

# trec_dl has different format, @lijun
def read_top_docs(q_id: str, documents_file: Path, top_file: Path, top_k: int=100) -> Tuple[List[str], List[str]]:
    top_docs_id, top_docs = [], []
    """ # no need to read document_file, top_file contains the passage already.
    with open(documents_file, encoding='utf-8', newline='')as f:
        for doc_id, doc in csv.reader(f, delimiter='\t'):
            docs[doc_id] = doc
    """
    with open(top_file, 'r')as f:
        for qid, did, query, doc in csv.reader(f, delimiter='\t'):
            if qid == q_id:
                top_docs_id.append(did)
                top_docs.append(doc)
    return top_docs_id[:top_k], top_docs[:top_k]
