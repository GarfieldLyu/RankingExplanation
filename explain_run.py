import explainers
from explainers import EXP_model
from utilities import dense_optimize, geno_solver, geno_solver_multi, gradients_expand, kendalltau_concord
from explain_base import Explain, device, analyzer
from utilities import preference_coverage
...
def explain_single(q_id: str, hparams: Dict[str, Any], exp_model: List[str]):
    """ generate expansion terms with a single matrix/explainer/method. """
    candidates_file = hparams['exp_fold'] / f"matrix_{hparams['Rerank_model']}" / f'{q_id}_candidates.json'
    candidates_tokens = load_json(candidates_file)
    if hparams['EXP_model'] == 'multi':
        matrix_file_1 = candidates_file.parent / f"{q_id}_matrix_{exp_model[0]}.pkl"  # change the name 
        matrix_file_2 = candidates_file.parent / f"{q_id}_matrix_{exp_model[1]}.pkl"
        matrix_file_3 = candidates_file.parent / f"{q_id}_matrix_{exp_model[2]}.pkl"
        matrixs = [load_pickle(matrix_file_1), load_pickle(matrix_file_2), load_pickle(matrix_file_3)]
    else:
        matrix_file = candidates_file.parent / f"{q_id}_matrix_{hparams['EXP_model']}.pkl"
        matrix = load_pickle(matrix_file)
    if hparams['EXP_model'] == 'saliency' and hparams['norm_saliency']:
        matrix = np.array(matrix)
